<?
class as_account_template
{
	var $id, $username, $as_username, $as_password;
	...
	function __construct()
	{
		...
		$this->_field_descs['as_username'] = array ("type" => "varchar(125)", "length" => "125", "gen_type" => "string");
		$this->_field_descs['as_password'] = array ("type" => "varchar(125)", "length" => "125", "gen_type" => "string");
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for as_account->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO as_accounts (`username`, `as_username`, `as_password`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->username)."', '".addslashes($this->as_username)."', '".addslashes($this->as_password)."')";
		}else{
			$raw_sql.= " VALUES ('$this->username', '$this->as_username', '$this->as_password')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		$raw_sql  = "UPDATE as_accounts SET ";
		if($addslashes) {
			$raw_sql.= "`username`='".addslashes($this->username)."', `as_username`='".addslashes($this->as_username)."', `as_password`='".addslashes($this->as_password)."'";
		}else{
			$raw_sql.= "`username`='$this->username', `as_username`='$this->as_username', `as_password`='$this->as_password'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE as_accounts SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE as_accounts SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE as_accounts SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM as_accounts WHERE 1
	...
	{
		if(!$order) $order = "" ;
		$select = "SELECT as_accounts.* FROM as_accounts ";
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['as_account'][$property]['max']) && $CONF['as_account'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['as_account'][$property]['max']) && $CONF['as_account'][$property]['max']){
					$min = ($CONF['as_account'][$property]['min'])? $CONF['as_account'][$property]['min'] : 0;
					$step = ($CONF['as_account'][$property]['step'])? $CONF['as_account'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['as_account'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['as_account'][$property]['size']))
					$size = $CONF['as_account'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['as_account'][$property]['past']))
						$past = $CONF['as_account'][$property]['past'];
					
					if (isset($CONF['as_account'][$property]['future']))
						$future = $CONF['as_account'][$property]['future'];

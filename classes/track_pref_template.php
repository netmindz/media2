<?
class track_pref_template
{
	var $id, $username, $time, $track_id, $score, $playcount, $last_update;
	...
	function __construct()
	{
		...
		$this->_field_descs['track_id'] = array ("type" => "int(11)", "length" => "11", "fk" => "track", "gen_type" => "int");
		$this->_field_descs['score'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
		$this->_field_descs['playcount'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for track_pref->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		if($this->time != (int)$this->time && $this->time!='NOW()' && $this->time!='NULL'){
			trigger_error("wrong type for track_pref->time",E_USER_WARNING);
			settype($this->time,"int");
		}//IF


		if($this->track_id != (int)$this->track_id && $this->track_id!='NOW()' && $this->track_id!='NULL'){
			trigger_error("wrong type for track_pref->track_id",E_USER_WARNING);
			settype($this->track_id,"int");
		}//IF


		if($this->score != (int)$this->score && $this->score!='NOW()' && $this->score!='NULL'){
			trigger_error("wrong type for track_pref->score",E_USER_WARNING);
			settype($this->score,"int");
		}//IF


		if($this->playcount != (int)$this->playcount && $this->playcount!='NOW()' && $this->playcount!='NULL'){
			trigger_error("wrong type for track_pref->playcount",E_USER_WARNING);
			settype($this->playcount,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO track_prefs (`username`, `time`, `track_id`, `score`, `playcount`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->username)."', '".addslashes($this->time)."', '".addslashes($this->track_id)."', '".addslashes($this->score)."', '".addslashes($this->playcount)."')";
		}else{
			$raw_sql.= " VALUES ('$this->username', '$this->time', '$this->track_id', '$this->score', '$this->playcount')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`username`='".addslashes($this->username)."', `time`='".addslashes($this->time)."', `track_id`='".addslashes($this->track_id)."', `score`='".addslashes($this->score)."', `playcount`='".addslashes($this->playcount)."'";
		}else{
			$raw_sql.= "`username`='$this->username', `time`='$this->time', `track_id`='$this->track_id', `score`='$this->score', `playcount`='$this->playcount'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE track_prefs SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE track_prefs SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE track_prefs SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	{
		if(!$order) $order = "" ;
		$select = "SELECT track_prefs.* FROM track_prefs ";
		...
		if(is_array($properties)) {
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child->_setPropertiesLinkages("track_pref", $this->id, array_keys($value));
				}
				else {
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
					$html .= $fk_class->createMatrix($input_name,"track_pref",$this->id);
				}
				else {
					ob_start();
					$extra .= " id=\"$html_id\" ";
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				}elseif (isset($CONF['track_pref'][$property]['max']) && $CONF['track_pref'][$property]['max']){
					$min = ($CONF['track_pref'][$property]['min'])? $CONF['track_pref'][$property]['min'] : 0;
					$step = ($CONF['track_pref'][$property]['step'])? $CONF['track_pref'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['track_pref'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['track_pref'][$property]['past']))
						$past = $CONF['track_pref'][$property]['past'];
					
					if (isset($CONF['track_pref'][$property]['future']))
						$future = $CONF['track_pref'][$property]['future'];

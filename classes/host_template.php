...
class host_template
{
	var $id, $hostname, $status, $share, $url_base, $last_online;
	...
	function __construct()
	{
		...
		$this->_field_descs['hostname'] = array ("type" => "varchar(50)", "length" => "50", "gen_type" => "string");
		$this->_field_descs['status'] = array ("type" => "enum('offline','online')", "default" => "online", "values" => array('offline','online',), "gen_type" => "enum");
		$this->_field_descs['share'] = array ("type" => "varchar(125)", "length" => "125", "gen_type" => "string");
		$this->_field_descs['url_base'] = array ("type" => "varchar(125)", "length" => "125", "gen_type" => "string");
		$this->_field_descs['last_online'] = array ("type" => "date", "gen_type" => "datetime");
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for host->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		if(!in_array($this->status,$this->_field_descs['status']['values']) && $this->status!='NULL') {
			if($this->status!='') trigger_error("Invalid enum value ".$this->status." for host->status, using default",E_USER_WARNING);
			$this->status = $this->_field_descs['status']['default'];
		}//IF

		
		$raw_sql  = "INSERT INTO hosts (`hostname`, `status`, `share`, `url_base`, `last_online`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->hostname)."', '".addslashes($this->status)."', '".addslashes($this->share)."', '".addslashes($this->url_base)."', '".addslashes($this->last_online)."')";
		}else{
			$raw_sql.= " VALUES ('$this->hostname', '$this->status', '$this->share', '$this->url_base', '$this->last_online')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		$raw_sql  = "UPDATE hosts SET ";
		if($addslashes) {
			$raw_sql.= "`hostname`='".addslashes($this->hostname)."', `status`='".addslashes($this->status)."', `share`='".addslashes($this->share)."', `url_base`='".addslashes($this->url_base)."', `last_online`='".addslashes($this->last_online)."'";
		}else{
			$raw_sql.= "`hostname`='$this->hostname', `status`='$this->status', `share`='$this->share', `url_base`='$this->url_base', `last_online`='$this->last_online'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE hosts SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE hosts SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE hosts SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	{
		if(!$order) $order = "" ;
		$select = "SELECT hosts.* FROM hosts ";
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['host'][$property]['max']) && $CONF['host'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['host'][$property]['max']) && $CONF['host'][$property]['max']){
					$min = ($CONF['host'][$property]['min'])? $CONF['host'][$property]['min'] : 0;
					$step = ($CONF['host'][$property]['step'])? $CONF['host'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['host'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['host'][$property]['size']))
					$size = $CONF['host'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['host'][$property]['past']))
						$past = $CONF['host'][$property]['past'];
					
					if (isset($CONF['host'][$property]['future']))
						$future = $CONF['host'][$property]['future'];

...
class host_template
{
	...
	function add() {
		...
		$raw_sql.= " VALUES ('".$this->database->escape($this->hostname)."', '".$this->database->escape($this->status)."', '".$this->database->escape($this->share)."', '".$this->database->escape($this->url_base)."', '".$this->database->escape($this->last_online)."')";
	...
	function update()
	{
		...
		$raw_sql.= "`hostname`='".$this->database->escape($this->hostname)."', `status`='".$this->database->escape($this->status)."', `share`='".$this->database->escape($this->share)."', `url_base`='".$this->database->escape($this->url_base)."', `last_online`='".$this->database->escape($this->last_online)."'";
		$raw_sql.= " WHERE 
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			...
			$sql = "UPDATE hosts SET $fieldname = '".$this->database->escape($this->$fieldname)."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM hosts WHERE id = '".$this->database->escape($id)."' ";
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk->createMatrix($input_name,"host",$this->id);

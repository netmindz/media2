<?
class type_pref_template
{
	var $id, $username, $time, $type, $type_id, $score, $scorecount, $last_update;
	...
	function __construct()
	{
		...
		$this->_field_descs['username'] = array ("type" => "varchar(50)", "length" => "50", "gen_type" => "string");
		$this->_field_descs['time'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
		$this->_field_descs['type'] = array ("type" => "enum('artist','album','genre')", "default" => "artist", "values" => array('artist','album','genre',), "gen_type" => "enum");
		$this->_field_descs['type_id'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
		$this->_field_descs['score'] = array ("type" => "float(5,2)", "gen_type" => "number");
		$this->_field_descs['scorecount'] = array ("type" => "int(11)", "length" => "11", "gen_type" => "int");
		$this->_field_descs['last_update'] = array ("type" => "timestamp", "gen_type" => "timestamp");
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for type_pref->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		if($this->time != (int)$this->time && $this->time!='NOW()' && $this->time!='NULL'){
			trigger_error("wrong type for type_pref->time",E_USER_WARNING);
			settype($this->time,"int");
		}//IF


		if(!in_array($this->type,$this->_field_descs['type']['values']) && $this->type!='NULL') {
			if($this->type!='') trigger_error("Invalid enum value ".$this->type." for type_pref->type, using default",E_USER_WARNING);
			$this->type = $this->_field_descs['type']['default'];
		}//IF

		if($this->type_id != (int)$this->type_id && $this->type_id!='NOW()' && $this->type_id!='NULL'){
			trigger_error("wrong type for type_pref->type_id",E_USER_WARNING);
			settype($this->type_id,"int");
		}//IF


		if($this->scorecount != (int)$this->scorecount && $this->scorecount!='NOW()' && $this->scorecount!='NULL'){
			trigger_error("wrong type for type_pref->scorecount",E_USER_WARNING);
			settype($this->scorecount,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO type_prefs (`username`, `time`, `type`, `type_id`, `score`, `scorecount`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->username)."', '".addslashes($this->time)."', '".addslashes($this->type)."', '".addslashes($this->type_id)."', '".addslashes($this->score)."', '".addslashes($this->scorecount)."')";
		}else{
			$raw_sql.= " VALUES ('$this->username', '$this->time', '$this->type', '$this->type_id', '$this->score', '$this->scorecount')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		$raw_sql  = "UPDATE type_prefs SET ";
		if($addslashes) {
			$raw_sql.= "`username`='".addslashes($this->username)."', `time`='".addslashes($this->time)."', `type`='".addslashes($this->type)."', `type_id`='".addslashes($this->type_id)."', `score`='".addslashes($this->score)."', `scorecount`='".addslashes($this->scorecount)."'";
		}else{
			$raw_sql.= "`username`='$this->username', `time`='$this->time', `type`='$this->type', `type_id`='$this->type_id', `score`='$this->score', `scorecount`='$this->scorecount'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE type_prefs SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE type_prefs SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE type_prefs SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM type_prefs WHERE 1
	...
	{
		if(!$order) $order = "" ;
		$select = "SELECT type_prefs.* FROM type_prefs ";
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child = new $child_class();
                        ...
                        $child->_setPropertiesLinkages("type_pref", $this->id, array_keys($value));
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk_class->createMatrix($input_name,"type_pref",$this->id);
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['type_pref'][$property]['max']) && $CONF['type_pref'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['type_pref'][$property]['max']) && $CONF['type_pref'][$property]['max']){
					$min = ($CONF['type_pref'][$property]['min'])? $CONF['type_pref'][$property]['min'] : 0;
					$step = ($CONF['type_pref'][$property]['step'])? $CONF['type_pref'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['type_pref'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['type_pref'][$property]['size']))
					$size = $CONF['type_pref'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['type_pref'][$property]['past']))
						$past = $CONF['type_pref'][$property]['past'];
					
					if (isset($CONF['type_pref'][$property]['future']))
						$future = $CONF['type_pref'][$property]['future'];

...
class type_pref_template
{
	...
	function add() {
		...
		$raw_sql.= " VALUES ('".$this->database->escape($this->username)."', '".$this->database->escape($this->time)."', '".$this->database->escape($this->type)."', '".$this->database->escape($this->type_id)."', '".$this->database->escape($this->score)."', '".$this->database->escape($this->scorecount)."')";
	...
	function update()
	{
		...
		$raw_sql.= "`username`='".$this->database->escape($this->username)."', `time`='".$this->database->escape($this->time)."', `type`='".$this->database->escape($this->type)."', `type_id`='".$this->database->escape($this->type_id)."', `score`='".$this->database->escape($this->score)."', `scorecount`='".$this->database->escape($this->scorecount)."'";
		$raw_sql.= " WHERE 
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			...
			$sql = "UPDATE type_prefs SET $fieldname = '".$this->database->escape($this->$fieldname)."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM type_prefs WHERE id = '".$this->database->escape($id)."' ";
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk->createMatrix($input_name,"type_pref",$this->id);

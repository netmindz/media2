...
{
	var $id, $mb_id, $name, $cd_number, $album_artist_id, $amazon_asin;
	...
	function __construct()
	{
		...
		$this->_field_descs['mb_id'] = array ("type" => "varchar(125)", "length" => "125", "gen_type" => "string");
		$this->_field_descs['name'] = array ("type" => "varchar(255)", "length" => "255", "gen_type" => "string");
		$this->_field_descs['cd_number'] = array ("type" => "int(2)", "length" => "2", "gen_type" => "int");
		$this->_field_descs['album_artist_id'] = array ("type" => "int(10) unsigned", "length" => "10", "fk" => "artist", "gen_type" => "int");
		$this->_field_descs['amazon_asin'] = array ("type" => "varchar(15)", "length" => "15", "gen_type" => "string");
	...
	function add($addslashes=0) {
		...
		if($this->cd_number != (int)$this->cd_number && $this->cd_number!='NOW()' && $this->cd_number!='NULL'){
			trigger_error("wrong type for album->cd_number",E_USER_WARNING);
			settype($this->cd_number,"int");
		}//IF


		if($this->album_artist_id != (int)$this->album_artist_id && $this->album_artist_id!='NOW()' && $this->album_artist_id!='NULL'){
			trigger_error("wrong type for album->album_artist_id",E_USER_WARNING);
			settype($this->album_artist_id,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO albums (`mb_id`, `name`, `cd_number`, `album_artist_id`, `amazon_asin`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->mb_id)."', '".addslashes($this->name)."', '".addslashes($this->cd_number)."', '".addslashes($this->album_artist_id)."', '".addslashes($this->amazon_asin)."')";
		}else{
			$raw_sql.= " VALUES ('$this->mb_id', '$this->name', '$this->cd_number', '$this->album_artist_id', '$this->amazon_asin')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`mb_id`='".addslashes($this->mb_id)."', `name`='".addslashes($this->name)."', `cd_number`='".addslashes($this->cd_number)."', `album_artist_id`='".addslashes($this->album_artist_id)."', `amazon_asin`='".addslashes($this->amazon_asin)."'";
		}else{
			$raw_sql.= "`mb_id`='$this->mb_id', `name`='$this->name', `cd_number`='$this->cd_number', `album_artist_id`='$this->album_artist_id', `amazon_asin`='$this->amazon_asin'";
		}//IF
...
//			$this = set_properties($this, $tmp, $addslashes,"get");
			...
			if($this->cd_number) { $this->DN = "$this->name - CD$this->cd_number" ; } else { $this->DN = $this->name; } 
		...
		if(is_array($properties)) {
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child->_setPropertiesLinkages("album", $this->id, array_keys($value));
				}
				else {
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
					$html .= $fk_class->createMatrix($input_name,"album",$this->id);
				}
				else {
					ob_start();
					$extra .= " id=\"$html_id\" ";
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['album'][$property]['max']) && $CONF['album'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['album'][$property]['max']) && $CONF['album'][$property]['max']){
					$min = ($CONF['album'][$property]['min'])? $CONF['album'][$property]['min'] : 0;
					$step = ($CONF['album'][$property]['step'])? $CONF['album'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['album'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['album'][$property]['size']))
					$size = $CONF['album'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['album'][$property]['past']))
						$past = $CONF['album'][$property]['past'];
					
					if (isset($CONF['album'][$property]['future']))
						$future = $CONF['album'][$property]['future'];

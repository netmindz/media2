...
class album_template
{
	...
	function add() {
		...
		$raw_sql.= " VALUES ('".$this->database->escape($this->mb_id)."', '".$this->database->escape($this->name)."', '".$this->database->escape($this->cd_number)."', '".$this->database->escape($this->album_artist_id)."', '".$this->database->escape($this->amazon_asin)."')";
	...
	function update()
	{
		...
		$raw_sql.= "`mb_id`='".$this->database->escape($this->mb_id)."', `name`='".$this->database->escape($this->name)."', `cd_number`='".$this->database->escape($this->cd_number)."', `album_artist_id`='".$this->database->escape($this->album_artist_id)."', `amazon_asin`='".$this->database->escape($this->amazon_asin)."'";
		$raw_sql.= " WHERE 
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	{
		$artist = new artist();
		$artist->get($this->album_artist_id);
		return($artist);
	}
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
						...
						$html .= $fk->createMatrix($input_name,"album",$this->id);

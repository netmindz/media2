...
class artist_template
{
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for artist->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO artists (`mb_id`, `name`)";
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE artists SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE artists SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE artists SET $fieldname = '".$this->$fieldname."'";
		...
		if(is_array($properties)) {
			...
			foreach ($properties as $key => $value) {
				if($this->_field_descs[$key]['gen_type'] == "many2many") {
					...
					$child->_setPropertiesLinkages("artist", $this->id, array_keys($value));
				}
				else {
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
					$html .= $fk_class->createMatrix($input_name,"artist",$this->id);
				}
				else {
					ob_start();
					$extra .= " id=\"$html_id\" ";
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				}elseif (isset($CONF['artist'][$property]['max']) && $CONF['artist'][$property]['max']){
					$min = ($CONF['artist'][$property]['min'])? $CONF['artist'][$property]['min'] : 0;
					$step = ($CONF['artist'][$property]['step'])? $CONF['artist'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['artist'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['artist'][$property]['past']))
						$past = $CONF['artist'][$property]['past'];
					
					if (isset($CONF['artist'][$property]['future']))
						$future = $CONF['artist'][$property]['future'];

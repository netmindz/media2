<?
class as_spool_item_template
{
	var $id, $as_account_id, $track_id, $played;
	 ...
	 */
	function as_spool_item_template()
	{
		$this->__construct();
	}//PHP4 constructor
	...
	function __construct()
	{
		...
		$this->_field_descs['as_account_id'] = array ("type" => "int(11)", "length" => "11", "fk" => "as_account", "gen_type" => "int");
		$this->_field_descs['track_id'] = array ("type" => "int(11)", "length" => "11", "fk" => "track", "gen_type" => "int");
		$this->_field_descs['played'] = array ("type" => "datetime", "gen_type" => "datetime");
	...
	function add() {
		...
		if($this->as_account_id != (int)$this->as_account_id && $this->as_account_id!='NOW()' && $this->as_account_id!='NULL'){
			trigger_error("wrong type for as_spool_item->as_account_id",E_USER_WARNING);
			settype($this->as_account_id,"int");
		}//IF


		if($this->track_id != (int)$this->track_id && $this->track_id!='NOW()' && $this->track_id!='NULL'){
			trigger_error("wrong type for as_spool_item->track_id",E_USER_WARNING);
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				}elseif (isset($CONF['as_spool_item'][$property]['max']) && $CONF['as_spool_item'][$property]['max']){
					$min = ($CONF['as_spool_item'][$property]['min'])? $CONF['as_spool_item'][$property]['min'] : 0;
					$step = ($CONF['as_spool_item'][$property]['step'])? $CONF['as_spool_item'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['as_spool_item'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['as_spool_item'][$property]['past']))
						$past = $CONF['as_spool_item'][$property]['past'];
					
					if (isset($CONF['as_spool_item'][$property]['future']))
						$future = $CONF['as_spool_item'][$property]['future'];

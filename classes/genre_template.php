...
class genre_template
{
	...
	function add($addslashes=0) {
		...
		if($this->id != (int)$this->id && $this->id!='NOW()' && $this->id!='NULL'){
			trigger_error("wrong type for genre->id",E_USER_WARNING);
			settype($this->id,"int");
		}//IF


		
		$raw_sql  = "INSERT INTO genres (`mb_id`, `name`)";
		
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->mb_id)."', '".addslashes($this->name)."')";
		}else{
			$raw_sql.= " VALUES ('$this->mb_id', '$this->name')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		$raw_sql  = "UPDATE genres SET ";
		if($addslashes) {
			$raw_sql.= "`mb_id`='".addslashes($this->mb_id)."', `name`='".addslashes($this->name)."'";
		}else{
			$raw_sql.= "`mb_id`='$this->mb_id', `name`='$this->name'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		if ($this->_field_descs[$fieldname]['gen_type'] == 'int' || $this->$fieldname == "NULL" || $this->$fieldname == "NOW()")
			$sql = "UPDATE genres SET $fieldname = ".$this->$fieldname;
		elseif ($addslashes)
			$sql = "UPDATE genres SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE genres SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM genres WHERE 1
	...
	function getList($where="", $order="", $limit="")
	{
		...
		$select = "SELECT genres.* FROM genres ";
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				if ($matches[1] ==1 || 			//a tiny int of display length 1 char is presumed to be a boolean
						(isset($CONF['genre'][$property]['max']) && $CONF['genre'][$property]['max']==1) ){		//or setting the max value to 1 presumes a boolean
				...
				}elseif (isset($CONF['genre'][$property]['max']) && $CONF['genre'][$property]['max']){
					$min = ($CONF['genre'][$property]['min'])? $CONF['genre'][$property]['min'] : 0;
					$step = ($CONF['genre'][$property]['step'])? $CONF['genre'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['genre'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'string' :
				...
				if (isset($CONF['genre'][$property]['size']))
					$size = $CONF['genre'][$property]['size'];
				elseif($maxlength <= 30)
					$size = $maxlength+1;
				elseif ($maxlength <= 50)
					$size = 40;
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['genre'][$property]['past']))
						$past = $CONF['genre'][$property]['past'];
					
					if (isset($CONF['genre'][$property]['future']))
						$future = $CONF['genre'][$property]['future'];

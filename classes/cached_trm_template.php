...
class cached_trm_template
{
	...
	function add($addslashes=0) {
		...
		if ($addslashes) {
				$raw_sql.= " VALUES ('".addslashes($this->path)."', '".addslashes($this->trm)."')";
		}else{
			$raw_sql.= " VALUES ('$this->path', '$this->trm')";
		}//IF slashes
	...
	function update($addslashes=0)
	{
		...
		if($addslashes) {
			$raw_sql.= "`path`='".addslashes($this->path)."', `trm`='".addslashes($this->trm)."'";
		}else{
			$raw_sql.= "`path`='$this->path', `trm`='$this->trm'";
		}//IF
	...
	* @desc Sets individual fields in the record, allowing special cases to be executed (eg. sess_expires), and leaving others unchanged.
	...
	function set($fieldname, $addslashes=0) {
		...
		elseif ($addslashes)
			$sql = "UPDATE cached_trms SET $fieldname = '".addslashes($this->$fieldname)."'";
		else
			$sql = "UPDATE cached_trms SET $fieldname = '".$this->$fieldname."'";
	...
	* @desc Wrapper that calls setProperties for the supplied pair and calls set()
	...
	function delete($id)
	{
		$sql = "DELETE FROM cached_trms WHERE 1
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			if (!$fk_class) {
				...
				if($this->_field_descs[$property]['gen_type'] == "many2many") {
					$html .= $fk_class->createMatrix($input_name,"cached_trm",$this->id);
				}
				else {
					ob_start();
					$extra .= " id=\"$html_id\" ";

<?
class dead_source_template
{
	var $id, $path, $reason, $notes, $updated;
	...
	function __construct()
	{
		...
		$this->_field_descs['reason'] = array ("type" => "varchar(125)", "length" => "125", "gen_type" => "string");
		$this->_field_descs['notes'] = array ("type" => "text", "gen_type" => "text");
		...
		if (isset($this->_field_descs[$property]['fk'] )){	//we have found a foreign key
			...
			switch ($this->_field_descs[$property]['gen_type']) {
			  ...
			  case 'number' :
				...
				}elseif (isset($CONF['dead_source'][$property]['max']) && $CONF['dead_source'][$property]['max']){
					$min = ($CONF['dead_source'][$property]['min'])? $CONF['dead_source'][$property]['min'] : 0;
					$step = ($CONF['dead_source'][$property]['step'])? $CONF['dead_source'][$property]['step'] : 1;
					if ($empty=='-None-')
						$empty = '--';
					$html.= createNumberSelect($input_name, $property_value, $min, $CONF['dead_source'][$property]['max'], $step, $empty);
					break;	//escape SWITCH statement
			  ...
			  case 'datetime' :
				...
				if (strpos($this->_field_descs[$property]['type'], "date") !== FALSE) {
					...
					if (isset($CONF['dead_source'][$property]['past']))
						$past = $CONF['dead_source'][$property]['past'];
					
					if (isset($CONF['dead_source'][$property]['future']))
						$future = $CONF['dead_source'][$property]['future'];
